...
export default function DoctorAppointmentDetailPage() {
  ...
  const handleComplete = async () => {
    try {
      ...
      fetchAppointment();
      // Optionally redirect to create medical exam
      // router.push(`/doctor/exam/new?appointmentId=${appointmentId}`);
    } catch (error) {
  ...
  if (!appointment) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/doctor/appointments" className="btn-primary mt-4 inline-flex">
  ...
  const canComplete = appointment.status === "SCHEDULED" || appointment.status === "IN_PROGRESS";
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-8">
      ...
      <div className="flex items-center gap-4">
        <Link href="/doctor/appointments" className="btn-icon">
      ...
      <Link
        ...
        className="backdrop-blur-lg bg-white/70 border border-white/50 rounded-2xl p-6 shadow-xl hover:border-blue-300 transition-all group block"
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-section">Thông tin bệnh nhân</h3>
          </div>
          <span className="text-xs text-blue-500 group-hover:underline">Xem hồ sơ →</span>
        </div>
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center text-xl font-semibold text-blue-600">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {canComplete && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 text-white font-medium hover:shadow-lg transition-all shadow-md"
          >
        ...
        {appointment.status === "COMPLETED" && (
          <Link
            href={`/doctor/exam/new?appointmentId=${appointmentId}`}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-medium hover:shadow-lg transition-all shadow-md"
          >
