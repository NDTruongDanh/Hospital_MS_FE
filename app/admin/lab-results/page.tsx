...
import { LabTestResult } from "@/services/lab.service";

export default function AdminLabResultsPage() {
  const { data } = useLabResults({ page: 0, size: 100 });
  const results = data?.content || [];
  const totalResults = results.length;
  const pendingCount = results.filter((r: LabTestResult) => r.status === "PENDING").length;
  const completedCount = results.filter((r: LabTestResult) => r.status === "COMPLETED").length;
  const abnormalCount = results.filter((r: LabTestResult) => r.isAbnormal).length;
